...
class apiSupervision{
  ...
  private function conectarServer(){
    ...
    if(!$this->objBd){
      $this->erroresConfSupervisionApi(300, "Error al conectar con la base de datos");
      return false;
    }

    if(!@mysqli_set_charset($this->objBd, "utf8")){
      $this->erroresConfSupervisionApi(300, "Error al configurar charset utf8");
      return false;
    }

    if(!@mysqli_query($this->objBd, "SET NAMES utf8")){
      $this->erroresConfSupervisionApi(300, "Error al preparar conversion de caracteres");
      return false;
    }
  ...
  private function prepararConsulta($sql){
    ...
    if(!$stmt){
      $this->erroresConfSupervisionApi(300, "Error al preparar la consulta");
      return false;
    }
  ...
  private function ejecutarConsultaPreparada($sql, $types, $params){
    ...
    if($types !== "" && !$this->bindStatementParams($stmt, $types, $params)){
      ...
      $this->erroresConfSupervisionApi(300, "Error al preparar los parametros de consulta");
      return false;
    }

    if(!@mysqli_stmt_execute($stmt)){
      @mysqli_stmt_close($stmt);
      $this->erroresConfSupervisionApi(300, "Error al ejecutar la consulta");
      return false;
    }
    ...
    if($result === false){
      @mysqli_stmt_close($stmt);
      $this->erroresConfSupervisionApi(300, "Error al obtener la informacion");
      return false;
    }
  ...
  private function validarContextoSesion(){
    if((int)$this->idCliente <= 0 || (int)$this->idUsuario <= 0){
      $this->erroresConfSupervisionApi(300, "Sesion no valida");
      return false;
    }
  ...
  private function resolverBinding($bindingDefinition){
    if(!isset($bindingDefinition["source"]) || !isset($bindingDefinition["name"])){
      $this->erroresConfSupervisionApi(300, "Configuracion de bindings invalida");
      return false;
    }

    if($bindingDefinition["source"] !== "property"){
      $this->erroresConfSupervisionApi(300, "Origen de binding no soportado");
      return false;
    }
  ...
  public function executeDefinedAction($actionDefinition){
    ...
    if(!isset($actionDefinition["execution"]) || !is_array($actionDefinition["execution"])){
      $this->erroresConfSupervisionApi(300, "Configuracion de ejecucion invalida");
      return;
    }

    $executionDefinition = $actionDefinition["execution"];
    if(!isset($executionDefinition["type"])){
      $this->erroresConfSupervisionApi(300, "Tipo de ejecucion invalido");
      return;
    }
    ...
    switch($executionDefinition["type"]){
      ...
      break;
      default:
        $this->erroresConfSupervisionApi(300, "Tipo de ejecucion no soportado");
      break;
    }
  }
  ...
  private function executeDefinedQuery($executionDefinition){
    if(!isset($executionDefinition["sql"]) || trim((string)$executionDefinition["sql"]) === ""){
      $this->erroresConfSupervisionApi(300, "Consulta no definida");
      return;
    }
  ...
  private function executeDefinedProcedure($executionDefinition){
    if(!isset($executionDefinition["sql"]) || trim((string)$executionDefinition["sql"]) === ""){
      $this->erroresConfSupervisionApi(300, "Procedimiento no definido");
      return;
    }
  ...
  private function executeDefinedApi($executionDefinition){
    if(!isset($executionDefinition["url"]) || trim((string)$executionDefinition["url"]) === ""){
      $this->erroresConfSupervisionApi(300, "URL de API no definida");
      return;
    }

    if(!function_exists("curl_init")){
      $this->erroresConfSupervisionApi(300, "cURL no disponible en servidor");
      return;
    }
    ...
    if($rawPayload === false){
      $this->erroresConfSupervisionApi(300, "No fue posible serializar el payload de API");
      return;
    }
    ...
    if($response === false){
      $this->erroresConfSupervisionApi(300, "Error al invocar API externa: ".$curlError);
      return;
    }

    if($curlErrno !== 0){
      $this->erroresConfSupervisionApi(300, "Error de cURL al invocar API externa: ".$curlError);
      return;
    }

    if($httpCode < 200 || $httpCode >= 300){
      $this->erroresConfSupervisionApi(300, "API externa respondio HTTP ".$httpCode);
      return;
    }
    ...
    if($decoded === null && json_last_error() !== JSON_ERROR_NONE){
      $this->erroresConfSupervisionApi(300, "Respuesta invalida de API externa");
      return;
    }

    $normalizedResponse = $this->decodeConfiguredJsonKeys($decoded, $executionDefinition);
    if($normalizedResponse === false && isset($this->registros["Error"])){
      return;
    }

    $responseData = $this->extractApiResponseData($normalizedResponse, $executionDefinition);
    if($responseData === false && isset($this->registros["Error"])){
      return;
    }

    if(is_array($responseData)){
      $this->registros = $responseData;
      return;
    }
  ...
  private function construirApiPayload($executionDefinition){
    ...
    if(!is_array($bodyDefinition)){
      $this->erroresConfSupervisionApi(300, "Configuracion de payload API invalida");
      return false;
    }
  ...
  private function decodeConfiguredJsonKeys($decoded, $executionDefinition){
    $jsonKeys = isset($executionDefinition["response_json_keys"]) ? $executionDefinition["response_json_keys"] : array();
    if(!is_array($jsonKeys) || empty($jsonKeys) || !is_array($decoded)){
      return $decoded;
    }

    foreach($jsonKeys as $jsonKey){
      if(!is_string($jsonKey) || !array_key_exists($jsonKey, $decoded)){
        continue;
      }

      if(!is_string($decoded[$jsonKey])){
        continue;
      }

      $trimmedValue = trim($decoded[$jsonKey]);
      if($trimmedValue === ""){
        $decoded[$jsonKey] = array();
        continue;
      }

      $parsedValue = json_decode($trimmedValue, true);
      if($parsedValue === null && json_last_error() !== JSON_ERROR_NONE){
        $this->erroresConfSupervisionApi(300, "Respuesta JSON invalida en la llave ".$jsonKey);
        return false;
      }

      $decoded[$jsonKey] = $parsedValue;
    }
  ...
  private function extractApiResponseData($decoded, $executionDefinition){
    ...
    if(!is_array($decoded) || !array_key_exists($responseDataKey, $decoded)){
      $this->erroresConfSupervisionApi(300, "Respuesta de API externa sin llave ".$responseDataKey);
      return false;
    }

    return $decoded[$responseDataKey];
  }
  ...
  private function erroresConfSupervisionApi($errorNo, $mensaje){
    switch ($errorNo) {
      case '200':
        $this->registros = array();
      break;
