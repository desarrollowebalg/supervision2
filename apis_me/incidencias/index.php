<?php
  ...
  require_once dirname(__FILE__)."/apiIncidencias.class.php";
  ...
  $incidenciasApi = new apiIncidencias($config_bd["host"],$config_bd["user"],$config_bd["pass"],$config_bd["bname"],$config_bd["port"]);

  hidratarContextoSesion($incidenciasApi, $moduleConfig["session_context"]);
  $incidenciasApi->configureSessionContext($moduleConfig["session_context"]);
  ...
  function hidratarContextoSesion($incidenciasApi, $sessionContext){
    foreach($sessionContext as $sessionRule){
      $value = $_SESSION[$sessionRule["session_key"]];
      if($sessionRule["type"] === "int"){
        $value = (int)$value;
      }
      $incidenciasApi->__set($sessionRule["property"], $value);
    }
  }
  ...
  function hidratarPropiedadesAccion($incidenciasApi, $actionDefinition, $resolvedParams){
    $params = isset($actionDefinition["params"]) ? $actionDefinition["params"] : array();
    foreach($params as $paramDefinition){
      if(isset($paramDefinition["target_property"]) && array_key_exists($paramDefinition["name"], $resolvedParams)){
        $incidenciasApi->__set($paramDefinition["target_property"], $resolvedParams[$paramDefinition["name"]]);
      }
    }
  }
