<?php
  ...
  require_once dirname(__FILE__)."/apiPayloads.class.php";
  ...
  $payloadsApi = new apiPayloads($config_bd["host"],$config_bd["user"],$config_bd["pass"],$config_bd["bname"],$config_bd["port"]);

  hidratarContextoSesion($payloadsApi, $moduleConfig["session_context"]);
  $resolvedParams = resolverParametrosAccion($route, $actionDefinition);
  hidratarPropiedadesAccion($payloadsApi, $actionDefinition, $resolvedParams);

  $payloadsApi->executeDefinedAction($actionDefinition);
  $resultado = $payloadsApi->__get("registros");
  ...
  function hidratarContextoSesion($payloadsApi, $sessionContext){
    foreach($sessionContext as $sessionRule){
      $value = $_SESSION[$sessionRule["session_key"]];
      if($sessionRule["type"] === "int"){
        $value = (int)$value;
      }
      $payloadsApi->__set($sessionRule["property"], $value);
    }
  }
  ...
  function hidratarPropiedadesAccion($payloadsApi, $actionDefinition, $resolvedParams){
    $params = isset($actionDefinition["params"]) ? $actionDefinition["params"] : array();
    foreach($params as $paramDefinition){
      if(isset($paramDefinition["target_property"]) && array_key_exists($paramDefinition["name"], $resolvedParams)){
        $payloadsApi->__set($paramDefinition["target_property"], $resolvedParams[$paramDefinition["name"]]);
      }
    }
  }
