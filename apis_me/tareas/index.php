<?php
  ...
  require_once dirname(__FILE__)."/apiTareas.class.php";
  ...
  $tareasApi = new apiTareas($config_bd["host"],$config_bd["user"],$config_bd["pass"],$config_bd["bname"],$config_bd["port"]);

  hidratarContextoSesion($tareasApi, $moduleConfig["session_context"]);
  $resolvedParams = resolverParametrosAccion($route, $actionDefinition);
  hidratarPropiedadesAccion($tareasApi, $actionDefinition, $resolvedParams);

  $tareasApi->executeDefinedAction($actionDefinition);
  $resultado = $tareasApi->__get("registros");
  ...
  function hidratarContextoSesion($tareasApi, $sessionContext){
    foreach($sessionContext as $sessionRule){
      $value = $_SESSION[$sessionRule["session_key"]];
      if($sessionRule["type"] === "int"){
        $value = (int)$value;
      }
      $tareasApi->__set($sessionRule["property"], $value);
    }
  }
  ...
  function hidratarPropiedadesAccion($tareasApi, $actionDefinition, $resolvedParams){
    $params = isset($actionDefinition["params"]) ? $actionDefinition["params"] : array();
    foreach($params as $paramDefinition){
      if(isset($paramDefinition["target_property"]) && array_key_exists($paramDefinition["name"], $resolvedParams)){
        $tareasApi->__set($paramDefinition["target_property"], $resolvedParams[$paramDefinition["name"]]);
      }
    }
  }
