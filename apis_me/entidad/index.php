<?php
  ...
  require_once dirname(__FILE__)."/apiEntidad.class.php";
  ...
  $entidadApi = new apiEntidad($config_bd["host"],$config_bd["user"],$config_bd["pass"],$config_bd["bname"],$config_bd["port"]);

  hidratarContextoSesion($entidadApi, $moduleConfig["session_context"]);
  $resolvedParams = resolverParametrosAccion($route, $actionDefinition);
  hidratarPropiedadesAccion($entidadApi, $actionDefinition, $resolvedParams);

  $entidadApi->executeDefinedAction($actionDefinition);
  $resultado = $entidadApi->__get("registros");
  ...
  if($actionName === "entity"){
    $resultado = validarEntidadAsignada($resultado);
  }
  ...
  function hidratarContextoSesion($entidadApi, $sessionContext){
    foreach($sessionContext as $sessionRule){
      $value = $_SESSION[$sessionRule["session_key"]];
      if($sessionRule["type"] === "int"){
        $value = (int)$value;
      }
      $entidadApi->__set($sessionRule["property"], $value);
    }
  }
  ...
  function hidratarPropiedadesAccion($entidadApi, $actionDefinition, $resolvedParams){
    $params = isset($actionDefinition["params"]) ? $actionDefinition["params"] : array();
    foreach($params as $paramDefinition){
      if(isset($paramDefinition["target_property"]) && array_key_exists($paramDefinition["name"], $resolvedParams)){
        $entidadApi->__set($paramDefinition["target_property"], $resolvedParams[$paramDefinition["name"]]);
      }
    }
  }
  ...
  function validarEntidadAsignada($resultado){
    $entityPayload = null;
    if(is_array($resultado) && isset($resultado[0]) && is_array($resultado[0])){
      $entityPayload = $resultado[0];
    } else if(is_array($resultado)){
      $entityPayload = $resultado;
    }

    $codEntity = 0;
    if(is_array($entityPayload) && isset($entityPayload["COD_ENTITY"])){
      $codEntity = (int)$entityPayload["COD_ENTITY"];
    }

    if($codEntity <= 0){
      responderApi(300, "No tiene ninguna unidad asignada para enviar evidencias");
      exit();
    }

    $_SESSION["COD_ENTITY"] = $codEntity;
    return array(
