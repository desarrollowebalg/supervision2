<?php
  ...
  require_once dirname(__FILE__)."/apiCuadrantes.class.php";
  ...
  $cuadrantesApi = new apiCuadrantes($config_bd["host"],$config_bd["user"],$config_bd["pass"],$config_bd["bname"],$config_bd["port"]);

  hidratarContextoSesion($cuadrantesApi, $moduleConfig["session_context"]);
  $resolvedParams = resolverParametrosAccion($route, $actionDefinition);
  hidratarPropiedadesAccion($cuadrantesApi, $actionDefinition, $resolvedParams);

  $cuadrantesApi->executeDefinedAction($actionDefinition);
  $resultado = $cuadrantesApi->__get("registros");
  ...
  function hidratarContextoSesion($cuadrantesApi, $sessionContext){
    foreach($sessionContext as $sessionRule){
      $value = $_SESSION[$sessionRule["session_key"]];
      if($sessionRule["type"] === "int"){
        $value = (int)$value;
      }
      $cuadrantesApi->__set($sessionRule["property"], $value);
    }
  }
  ...
  function hidratarPropiedadesAccion($cuadrantesApi, $actionDefinition, $resolvedParams){
    $params = isset($actionDefinition["params"]) ? $actionDefinition["params"] : array();
    foreach($params as $paramDefinition){
      if(isset($paramDefinition["target_property"]) && array_key_exists($paramDefinition["name"], $resolvedParams)){
        $cuadrantesApi->__set($paramDefinition["target_property"], $resolvedParams[$paramDefinition["name"]]);
      }
    }
  }
