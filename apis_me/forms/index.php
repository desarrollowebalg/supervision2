<?php
  ...
  require_once dirname(__FILE__)."/apiForms.class.php";
  ...
  $formsApi = new apiForms($config_bd["host"],$config_bd["user"],$config_bd["pass"],$config_bd["bname"],$config_bd["port"]);

  hidratarContextoSesion($formsApi, $moduleConfig["session_context"]);
  $resolvedParams = resolverParametrosAccion($route, $actionDefinition);
  hidratarPropiedadesAccion($formsApi, $actionDefinition, $resolvedParams);

  $formsApi->executeDefinedAction($actionDefinition);
  $resultado = $formsApi->__get("registros");
  ...
  function hidratarPropiedadesAccion($formsApi, $actionDefinition, $resolvedParams){
    $params = isset($actionDefinition["params"]) ? $actionDefinition["params"] : array();
    foreach($params as $paramDefinition){
      if(isset($paramDefinition["target_property"]) && array_key_exists($paramDefinition["name"], $resolvedParams)){
        $formsApi->__set($paramDefinition["target_property"], $resolvedParams[$paramDefinition["name"]]);
      }
    }
  }
