<?php
  ...
  require_once dirname(__FILE__)."/apiPdiV3.class.php";
  ...
  $pdiApi = new apiPdiV3($config_bd["host"],$config_bd["user"],$config_bd["pass"],$config_bd["bname"],$config_bd["port"]);

  hidratarContextoSesion($pdiApi, $moduleConfig["session_context"]);
  $resolvedParams = resolverParametrosAccion($route, $actionDefinition);
  hidratarPropiedadesAccion($pdiApi, $actionDefinition, $resolvedParams);

  $pdiApi->executeDefinedAction($actionDefinition);
  $resultado = $pdiApi->__get("registros");
  ...
  function hidratarPropiedadesAccion($pdiApi, $actionDefinition, $resolvedParams){
    $params = isset($actionDefinition["params"]) ? $actionDefinition["params"] : array();
    foreach($params as $paramDefinition){
      if(isset($paramDefinition["target_property"]) && array_key_exists($paramDefinition["name"], $resolvedParams)){
        $pdiApi->__set($paramDefinition["target_property"], $resolvedParams[$paramDefinition["name"]]);
      }
    }
  }
