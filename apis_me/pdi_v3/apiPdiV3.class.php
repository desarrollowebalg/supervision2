...
class apiPdiV3{
  ...
  private function conectarServer(){
    ...
    if(!$this->objBd){
      $this->erroresConfPdiApi(300, "Error al conectar con la base de datos");
      return false;
    }

    if(!@mysqli_set_charset($this->objBd, "utf8")){
      $this->erroresConfPdiApi(300, "Error al configurar charset utf8");
      return false;
    }

    if(!@mysqli_query($this->objBd, "SET NAMES utf8")){
      $this->erroresConfPdiApi(300, "Error al preparar conversion de caracteres");
      return false;
    }
  ...
  private function prepararConsulta($sql){
    ...
    if(!$stmt){
      $this->erroresConfPdiApi(300, "Error al preparar la consulta");
      return false;
    }
  ...
  private function ejecutarConsultaPreparada($sql, $types, $params){
    ...
    if($types !== "" && !$this->bindStatementParams($stmt, $types, $params)){
      ...
      $this->erroresConfPdiApi(300, "Error al preparar los parametros de consulta");
      return false;
    }

    if(!@mysqli_stmt_execute($stmt)){
      @mysqli_stmt_close($stmt);
      $this->erroresConfPdiApi(300, "Error al ejecutar la consulta");
      return false;
    }
    ...
    if($result === false){
      @mysqli_stmt_close($stmt);
      $this->erroresConfPdiApi(300, "Error al obtener la informacion");
      return false;
    }
  ...
  private function validarContextoSesion(){
    if((int)$this->idCliente <= 0 || (int)$this->idUsuario <= 0){
      $this->erroresConfPdiApi(300, "Sesion no valida");
      return false;
    }
  ...
  private function resolverBinding($bindingDefinition){
    if(!isset($bindingDefinition["source"]) || !isset($bindingDefinition["name"])){
      $this->erroresConfPdiApi(300, "Configuracion de bindings invalida");
      return false;
    }

    if($bindingDefinition["source"] !== "property"){
      $this->erroresConfPdiApi(300, "Origen de binding no soportado");
      return false;
    }
  ...
  public function executeDefinedAction($actionDefinition){
    ...
    if(!isset($actionDefinition["execution"]) || !is_array($actionDefinition["execution"])){
      $this->erroresConfPdiApi(300, "Configuracion de ejecucion invalida");
      return;
    }

    $executionDefinition = $actionDefinition["execution"];
    if(!isset($executionDefinition["type"])){
      $this->erroresConfPdiApi(300, "Tipo de ejecucion invalido");
      return;
    }
    ...
    switch($executionDefinition["type"]){
      ...
      break;
      default:
        $this->erroresConfPdiApi(300, "Tipo de ejecucion no soportado");
      break;
    }
  }
  ...
  private function executeDefinedQuery($executionDefinition){
    if(!isset($executionDefinition["sql"]) || trim((string)$executionDefinition["sql"]) === ""){
      $this->erroresConfPdiApi(300, "Consulta no definida");
      return;
    }
  ...
  private function executeDefinedProcedure($executionDefinition){
    if(!isset($executionDefinition["sql"]) || trim((string)$executionDefinition["sql"]) === ""){
      $this->erroresConfPdiApi(300, "Procedimiento no definido");
      return;
    }
  ...
  private function erroresConfPdiApi($errorNo, $mensaje){
    switch ($errorNo) {
      case '200':
        $this->registros = array();
      break;
